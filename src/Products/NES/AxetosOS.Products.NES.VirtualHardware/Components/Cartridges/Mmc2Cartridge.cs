...
/// <summary>
/// Mapper 9 / Nintendo MMC2 / PxROM cartridge hardware. MMC2 presents one
/// switchable 8 KiB PRG-ROM window followed by three fixed-last 8 KiB windows
/// and two 4 KiB CHR-ROM pattern-table windows. Each CHR window has an FD and
/// FE bank register; PPU reads of the MMC2 trigger addresses clock the matching
/// package latch after the current read, selecting which register drives the
/// corresponding CHR-ROM address pins. Mirroring is mapper-controlled, there
/// is no PRG RAM, IRQ, CHR RAM, or CPU/ROM bus conflict.
/// </summary>
public sealed class Mmc2Cartridge : VirtualHardwareComponent, IReplaceableCartridgeHardware,
    ICompiledBusTargetProvider, ICompiledCombinationalComponent, ICompiledStaticCombinationalComponent
{
    ...
    public Mmc2Cartridge(string componentId) : base(componentId)
    {
    ...
    public void LoadImage(VirtualHardwareNesRomImage image)
    {
        ...
        if (image.MapperNumber != 9)
            throw new NotSupportedException($"Mapper {image.MapperNumber} is not MMC2/PxROM hardware.");
        if (image.SubmapperNumber is > 0)
            throw new NotSupportedException($"Mapper 9 submapper {image.SubmapperNumber} is not defined for the Nintendo MMC2/PxROM package.");
        if (image.Mirroring == VirtualHardwareNesMirroring.FourScreen)
            throw new NotSupportedException("MMC2/PxROM does not provide four-screen nametable RAM.");
        if (image.PrgRom.Length < 4 * PrgBankSize || image.PrgRom.Length % PrgBankSize != 0)
            throw new ArgumentException("MMC2 PRG ROM must contain at least four whole 8 KiB banks.", nameof(image));
        var prgBanks = image.PrgRom.Length / PrgBankSize;
        if (prgBanks > 16 || !IsPowerOfTwo(prgBanks))
            throw new NotSupportedException("MMC2 exposes a power-of-two PRG-ROM population of at most sixteen 8 KiB banks (128 KiB).");
        if (image.ChrRom.Length < 2 * ChrBankSize || image.ChrRom.Length % ChrBankSize != 0)
            throw new NotSupportedException("MMC2/PxROM requires CHR ROM in whole 4 KiB banks; CHR-RAM boards are distinct hardware.");
        var chrBanks = image.ChrRom.Length / ChrBankSize;
        if (chrBanks > 32 || !IsPowerOfTwo(chrBanks))
            throw new NotSupportedException("MMC2 exposes a power-of-two CHR-ROM population of at most thirty-two 4 KiB banks (128 KiB).");
        if (image.HasExplicitRamSizes && image.TotalPrgRamSizeBytes != 0)
            throw new NotSupportedException("MMC2/PxROM has no PRG RAM.");
        if (image.HasExplicitRamSizes && image.TotalChrRamSizeBytes != 0)
            throw new NotSupportedException("MMC2/PxROM has no CHR RAM.");
        if (image.HasBatteryBackedMemory)
            throw new NotSupportedException("MMC2/PxROM has no battery-backed cartridge RAM.");
    ...
    private void ApplyResetState()
    {
        ...
        // MMC2 has no documented reset pin/state for these latches. AxetosOS
    ...
    private void UpdateCpuPort()
    {
        ...
        var address = (ushort)(0x8000 | (rawAddress & 0x7FFF));
        var romSelected = CpuM2.SampledLevel == DigitalLevel.High && CpuRomSelectBar.SampledLevel == DigitalLevel.Low;
        _cpuCycleRomSelected = romSelected;
        _cpuCycleAddress = address;
        _cpuReadAddressSelected = romSelected && CpuReadWrite.SampledLevel == DigitalLevel.High;
    ...
    private void CompleteCpuTransaction()
    {
        ...
        WriteMapperRegister(_cpuCycleAddress, (byte)rawData);
    }
    ...
    {
        var window = (address - 0x8000) >> 13;
        var bank = window == 0
            ? (_prgBankRegister & _prgBankMask)
            : ((_prg.Length / PrgBankSize) - 4 + window);
        ...
        out CompiledDriveState drive)
    {
        ...
        // MMC2 mirroring is mapper-controlled, so CIRAM A10 is intentionally
