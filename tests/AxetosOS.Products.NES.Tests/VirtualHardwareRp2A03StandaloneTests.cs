...
public sealed class VirtualHardwareRp2A03StandaloneTests
{
    [Fact]
    public void Rp2A03_divides_the_external_master_clock_into_M2()
    {
        var board = new VirtualHardwareBoard("chiptest.rp2a03.clock");
        ...
        PulseMasterClock(clock, simulator, 5);
        ...
        PulseMasterClock(clock, simulator, 6);
        Assert.Equal(DigitalLevel.Low, m2.Level);
        Assert.Equal(1UL, chip.RisingEdgeCount);
        Assert.Equal(12UL, chip.MasterClockRisingEdgeCount);
    }

    [Fact]
    public void Rp2A03_releases_package_outputs_without_power()
    {
        var board = new VirtualHardwareBoard("chiptest.rp2a03.power");
        var chip = board.Add(new Rp2A03("U1"));
        ...
        var m2 = board.Connect("observe.M2", chip.M2);
        var readWrite = board.Connect("observe.RW", chip.ReadWrite);
        var address = chip.Address.Pins.Select((pin, bit) => board.Connect($"observe.A{bit}", pin)).ToArray();
        ...
        Assert.Equal(DigitalLevel.Unknown, readWrite.Level);
        Assert.All(address, net => Assert.Equal(DigitalLevel.Unknown, net.Level));
    }

    private static void PulseMasterClock(
