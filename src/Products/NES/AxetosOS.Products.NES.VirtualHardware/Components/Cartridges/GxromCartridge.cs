...
/// <summary>
/// Mapper-66/GxROM replaceable cartridge hardware. The cartridge owns
/// one switchable 32 KiB PRG-ROM window, one switchable 8 KiB CHR-ROM window,
/// an end-of-M2 74HC161 quad latch, fixed nametable wiring and the board-local
/// CPU/ROM bus conflict. Motherboards and the generic hardware
/// compiler see only connector pins plus product-agnostic compilable facets.
/// </summary>
public sealed class GxromCartridge : VirtualHardwareComponent, IReplaceableCartridgeHardware,
    ICompiledBusTargetProvider, ICompiledCombinationalComponent, ICompiledStaticCombinationalComponent
{
    ...
    public GxromCartridge(string componentId) : base(componentId)
    {
    ...
    public void LoadImage(VirtualHardwareNesRomImage image)
    {
        ...
        if (image.MapperNumber != 66)
            throw new NotSupportedException($"Mapper {image.MapperNumber} is not GxROM.");

        if (image.PrgRom.Length < PrgBankSize || image.PrgRom.Length > MaximumPrgSize ||
            image.PrgRom.Length % PrgBankSize != 0)
            throw new ArgumentException(
                "GxROM PRG ROM must contain one to four whole 32 KiB banks.", nameof(image));

        var prgBankCount = image.PrgRom.Length / PrgBankSize;
        if ((prgBankCount & (prgBankCount - 1)) != 0)
            throw new NotSupportedException(
                "GxROM PRG ROM must expose a power-of-two number of 32 KiB banks so latch outputs map directly to ROM address pins.");

        if (image.ChrRom.Length < ChrBankSize || image.ChrRom.Length > MaximumChrSize ||
            image.ChrRom.Length % ChrBankSize != 0)
            throw new ArgumentException(
                "GxROM CHR ROM must contain one to four whole 8 KiB banks.", nameof(image));

        var chrBankCount = image.ChrRom.Length / ChrBankSize;
        if ((chrBankCount & (chrBankCount - 1)) != 0)
            throw new NotSupportedException(
                "GxROM CHR ROM must expose a power-of-two number of 8 KiB banks so latch outputs map directly to ROM address pins.");

        if (image.Mirroring == VirtualHardwareNesMirroring.FourScreen)
            throw new NotSupportedException(
                "Standard GxROM boards use fixed horizontal or vertical CIRAM wiring and have no cartridge nametable RAM.");
        if (image.HasBatteryBackedMemory)
            throw new NotSupportedException("Standard GxROM hardware has no battery-backed cartridge memory.");
        if (image.HasExplicitRamSizes && image.TotalPrgRamSizeBytes != 0)
            throw new NotSupportedException("Standard GxROM hardware has no PRG RAM/NVRAM chip.");
        if (image.HasExplicitRamSizes && image.TotalChrRamSizeBytes != 0)
            throw new NotSupportedException("Standard GxROM uses CHR ROM rather than CHR RAM/NVRAM.");

        // Mapper 66 has no standardized NES 2.0 submapper allocation for a
        // different conflict or banking variant. Refuse unknown non-zero
        // variants instead of silently substituting other discrete wiring.
        if (image.SubmapperNumber is int submapper && submapper != 0)
            throw new NotSupportedException(
                $"Mapper 66 submapper {submapper} has no defined GxROM board mapping in this hardware model.");
    ...
    private void RefreshDecodedBankBases()
    {
        _selectedPrgBankBase = _prg.Length == 0
            ? 0
            : ((_bankRegister >> 4) & _prgBankSelectMask) * PrgBankSize;
    ...
    private void WriteMapperRegister(ushort address, byte cpuData)
    {
        ...
        // GNROM physically latches CPU D4-D5 onto the two PRG-ROM bank
        // address lines and D0-D1 onto the two CHR-ROM bank address lines.
        // Unconnected latch/address lines are absent rather than software
        // modulo-normalized, so retain only outputs that exist on the fitted
        // board and ROM packages.
        _bankRegister = (byte)(effectiveData & ((_prgBankSelectMask << 4) | _chrBankSelectMask));
        RefreshDecodedBankBases();
    }
