...
public sealed class VirtualHardwareRp2C02StandaloneTests
{
    ...
    [Fact]
    public void Rendering_pipeline_performs_two_real_dummy_nametable_bus_reads_at_scanline_end()
    {
        var fixture = new Fixture();
        fixture.WriteRegister(1, 0x08);

        var addresses = fixture.PulseClockWithVram(341, _ => 0x00);

        Assert.Equal(2UL, fixture.Chip.DummyNametableFetchCount);
        Assert.True(addresses.Count(address => address is >= 0x2000 and < 0x3000) >= 36);
    }

    [Fact]
    public void Diagnostic_snapshot_hashes_retained_palette_oam_and_pipeline_state_without_mutating_execution()
    {
        var fixture = new Fixture();
        var before = fixture.Chip.InspectDiagnosticState();

        fixture.WriteRegister(3, 0x20);
        fixture.WriteRegister(4, 0x5A);
        fixture.WriteVramAddress(0x3F03);
        fixture.WriteRegister(7, 0x2A);
        var after = fixture.Chip.InspectDiagnosticState();

        Assert.NotEqual(before.PrimaryOamHash, after.PrimaryOamHash);
        Assert.NotEqual(before.PaletteRamHash, after.PaletteRamHash);
        Assert.NotEqual(before.PipelineStateHash, after.PipelineStateHash);
        Assert.Equal((byte)0x5A, fixture.Chip.InspectOam(0x20));
        Assert.Equal((byte)0x2A, fixture.Chip.InspectPalette(0x3F03));
    }
