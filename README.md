...
## v0.61.0 — RP2A03 completion sweep

Version 0.61.0 completes the remaining high-value digital behavior of the standalone RP2A03 package before motherboard integration. BRK now performs its real padding read, stacks PC+2 with B set, and enters through the IRQ/BRK vector; the twelve KIL/JAM opcodes now provide the reset-only halted state previously used as a test sentinel. The CPU core also implements the stable NMOS unofficial opcode families LAX, SAX, SLO, RLA, SRE, RRA, DCP, ISC, ANC, ALR, ARR and AXS, the alternate SBC opcode, and the documented read-NOP encodings while preserving their external bus accesses.

Indexed legal and unofficial instructions now apply the extra dummy read only when required: ordinary reads pay the additional cycle only on page crossing, while stores and read-modify-write operations always perform it. Taken branches now perform the second dummy read when crossing a page. Existing DMC/OAM arbitration, 3/4-cycle DMC stalls, reset sequencing, IRQ/NMI stack timing, controller I/O, frame sequencer, all five APU channels and nonlinear mixing remain package-internal and pin-driven.

The remaining limitations are explicit: unstable silicon-dependent opcodes (XAA, AHX, TAS, SHX, SHY and LAS edge behavior), transistor-level power-up randomness, analog audio filtering, and sub-cycle metastability are outside the deterministic digital package scope. No motherboard, PPU, cartridge, RAM or emulator callback was introduced.

## v0.62.0 — discrete logic and SRAM completion sweep
...
## v0.69.0 — PAL NES motherboard assembly
...
## v0.69.0 — shared virtual ROM slot and regional motherboard selector

- Adds one normalized ROM insertion point independent of physical 60-pin/72-pin cartridge packaging.
- Selects exactly one Famicom, NTSC NES, or PAL NES motherboard from ROM metadata or manual override.
