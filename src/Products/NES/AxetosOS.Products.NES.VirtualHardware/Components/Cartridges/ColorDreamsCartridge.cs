...
/// <summary>
/// Mapper-11/Color Dreams replaceable cartridge hardware. The cartridge owns
/// one switchable 32 KiB PRG-ROM window, one switchable 8 KiB CHR-ROM window,
/// an end-of-M2 74LS377-style bank latch, fixed nametable wiring and the
/// board-local CPU/ROM bus conflict. Motherboards and the generic hardware
/// compiler see only connector pins plus product-agnostic compilable facets.
/// </summary>
public sealed class ColorDreamsCartridge : VirtualHardwareComponent, IReplaceableCartridgeHardware,
    ICompiledBusTargetProvider, ICompiledCombinationalComponent, ICompiledStaticCombinationalComponent
{
    ...
    private const int MaximumPrgSize = 128 * 1024;
    private const int ChrBankSize = 8 * 1024;
    private const int MaximumChrSize = 128 * 1024;
    ...
    public ColorDreamsCartridge(string componentId) : base(componentId)
    {
    ...
    public void LoadImage(VirtualHardwareNesRomImage image)
    {
        ...
        if (image.MapperNumber != 11)
            throw new NotSupportedException($"Mapper {image.MapperNumber} is not Color Dreams.");

        if (image.PrgRom.Length < PrgBankSize || image.PrgRom.Length > MaximumPrgSize ||
            image.PrgRom.Length % PrgBankSize != 0)
            throw new ArgumentException(
                "Color Dreams PRG ROM must contain one to four whole 32 KiB banks.", nameof(image));

        var prgBankCount = image.PrgRom.Length / PrgBankSize;
        if ((prgBankCount & (prgBankCount - 1)) != 0)
            throw new NotSupportedException(
                "Color Dreams PRG ROM must expose a power-of-two number of 32 KiB banks so latch outputs map directly to ROM address pins.");

        if (image.ChrRom.Length < ChrBankSize || image.ChrRom.Length > MaximumChrSize ||
            image.ChrRom.Length % ChrBankSize != 0)
            throw new ArgumentException(
                "Color Dreams CHR ROM must contain one to sixteen whole 8 KiB banks.", nameof(image));

        var chrBankCount = image.ChrRom.Length / ChrBankSize;
        if ((chrBankCount & (chrBankCount - 1)) != 0)
            throw new NotSupportedException(
                "Color Dreams CHR ROM must expose a power-of-two number of 8 KiB banks so latch outputs map directly to ROM address pins.");

        if (image.Mirroring == VirtualHardwareNesMirroring.FourScreen)
            throw new NotSupportedException(
                "Standard Color Dreams boards use fixed horizontal or vertical CIRAM wiring and have no cartridge nametable RAM.");
        if (image.HasBatteryBackedMemory)
            throw new NotSupportedException("Standard Color Dreams hardware has no battery-backed cartridge memory.");
        if (image.HasExplicitRamSizes && image.TotalPrgRamSizeBytes != 0)
            throw new NotSupportedException("Standard Color Dreams hardware has no PRG RAM/NVRAM chip.");
        if (image.HasExplicitRamSizes && image.TotalChrRamSizeBytes != 0)
            throw new NotSupportedException("Standard Color Dreams uses CHR ROM rather than CHR RAM/NVRAM.");

        // Mapper 11 currently has no standardized NES 2.0 submapper allocation
        // for the rare no-conflict prototype board. Refuse unknown non-zero
        // variants instead of silently substituting different physical wiring.
        if (image.SubmapperNumber is int submapper && submapper != 0)
            throw new NotSupportedException(
                $"Mapper 11 submapper {submapper} has no defined Color Dreams board mapping in this hardware model.");

        _busConflictsEnabled = true;
    ...
    private void RefreshDecodedBankBases()
    {
        _selectedPrgBankBase = _prg.Length == 0
            ? 0
            : (_bankRegister & _prgBankSelectMask) * PrgBankSize;
        _selectedChrBankBase = _chr.Length == 0
            ? 0
            : ((_bankRegister >> 4) & _chrBankSelectMask) * ChrBankSize;
    }
    ...
    private void WriteMapperRegister(ushort address, byte cpuData)
    {
        ...
        // The full octal latch retains the bus value. D0-D1 feed PRG bank
        // address lines, D4-D7 feed CHR bank address lines, and D2-D3 belong
        // to the original lockout-defeat circuit rather than memory banking.
        _bankRegister = effectiveData;
        RefreshDecodedBankBases();
    }
